...
async def _repair_custom_emoji_entities(bot, text: str, entities):
    ...
    for ent in custom:
        ...
        if alt:
            ...
            end_py=py_from_u16(text,int(ent.offset)+int(ent.length))
            # اگر entity ذخیره‌شده همین حالا دقیقاً روی fallback خودش قرار دارد،
            # اصلاً جابه‌جایش نکن. جابه‌جایی کورکورانه‌ی نسخه‌ی قبلی روی متن‌هایی
            # با چند Emoji مشابه (مثل پروفایل) باعث می‌شد بعضی entityها روی هم بیفتند
            # و در نتیجه فقط بخشی از Premium Emojiها نمایش داده شوند.
            if text[old_py:end_py] == alt:
                result.append(ent)
                continue
        ...
        # اگر Bot API نتوانست اطلاعات alt این Premium Emoji را برگرداند،
        # entity معتبرِ ذخیره‌شده را حذف نکن. در متن‌هایی که از ادمین ذخیره
        # شده‌اند offset/length از ابتدا بر اساس UTF-16 خود Telegram هستند و
        # تا وقتی متن تغییر نکرده، همین entity کاملاً معتبر است. حذف کردن آن
        # باعث می‌شد در یک متن چند ایموجی پرمیوم فقط بعضی از آن‌ها نمایش داده شوند.
        result.append(ent)
...
async def send_rich(bot, chat_id, value=None, **kwargs):
    ...
    if entities:
        # برای متن‌های قابل ویرایش که Entityها از خود Telegram ذخیره شده‌اند،
        # اگر offset/length همین حالا برای متن نهایی معتبر است، دقیقاً همان
        # custom_emoji_id را نگه می‌داریم. اجرای repair روی لاگ‌های سفارشی می‌توانست
        # Premium Emoji انتخاب‌شده توسط ادمین را با fallback معمولی جابه‌جا کند.
        normalized=_sanitize_entities_for_text(text, entities)
        kwargs.pop('parse_mode',None)
        try:
...
async def show_menu_with_sticker(
    ...
    # بعضی متن‌های منو قبل از نمایش placeholder دارند یا در مسیرهای مختلف
    # ساخته می‌شوند؛ بنابراین entityهای ذخیره‌شده ممکن است نسبت به متن نهایی
    # نیاز به repair داشته باشند. نسخه‌ی جدید repair ابتدا entityهایی را که
    # دقیقاً روی fallback صحیح خود هستند دست‌نخورده نگه می‌دارد و فقط entity
    # واقعاً جابه‌جا شده را اصلاح می‌کند؛ پس هم منوهای عمومی و هم پروفایل
    # Premium Emoji کامل خود را حفظ می‌کنند.
    if entities:
    ...
    for tid in targets:
        try:
            if getattr(text, "entities", None):
                await send_rich(bot, int(tid), text, reply_markup=reply_markup, parse_mode=parse_mode)
            else:
                await bot.send_message(int(tid), str(text), reply_markup=reply_markup, parse_mode=parse_mode)
            sent.append(str(tid))
        except Exception:
            pass
    return sent
