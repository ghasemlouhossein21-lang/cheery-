...
async def _repair_custom_emoji_entities(bot, text: str, entities):
    ...
    for ent in custom:
        ...
        # اگر Telegram نتوانست fallback emoji را برگرداند، entity معتبر را نگه می‌داریم.
        if not alt:
            result.append(ent)
            continue
        ...
        hits=[]; start_pos=0
        while True:
            pos=text.find(alt,start_pos)
            if pos<0: break
            hits.append(pos); start_pos=pos+1
        if hits:
            pos=min(hits,key=lambda x:abs(x-old_py))
            result.append(ent.model_copy(update={'offset':u16(text[:pos]),'length':u16(alt)}))
            continue

        # اگر fallback در متن پیدا نشد، entity اصلی را حذف نکن.
        result.append(ent)
