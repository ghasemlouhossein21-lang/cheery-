...
async def send_rich(bot, chat_id, value=None, **kwargs):
    ...
    repair_custom_emoji = bool(kwargs.pop("_repair_custom_emoji", False))
    if entities:
        # به‌صورت پیش‌فرض رفتار send_rich دست‌نخورده می‌ماند؛ فقط لاگ سفارش
        # می‌تواند صراحتاً درخواست تطبیق دوباره‌ی custom_emojiها را بدهد.
        normalized = (
            await _repair_custom_emoji_entities(bot, text, entities)
            if repair_custom_emoji and bot
            else _sanitize_entities_for_text(text, entities)
        )
        kwargs.pop('parse_mode',None)
        try:
