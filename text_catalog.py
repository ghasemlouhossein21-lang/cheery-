...
def _render_with_entities(template: str, entities: list[dict], values: dict, preserve_dynamic_entities: bool = False) -> RichText:
    """Render a template while remapping Telegram entities safely.

    Custom/Premium Emoji entities belong to the literal text typed by the admin.
    Placeholder replacement changes offsets, so an entity is copied only when its
    complete UTF-16 span is inside a literal segment. Entities that overlap a
    placeholder are intentionally dropped because applying them to dynamic data
    is unsafe. This keeps Premium Emoji entities on the exact emoji they were
    ...
    parts = []
    literal_ranges = []  # source UTF-16 [start,end) -> output UTF-16 [start,end)
    dynamic_entities = []
    src_pos = 0
    out_pos = 0
    ...
    for literal, field, spec, conv in formatter.parse(template):
        if literal:
            ...
            literal_ranges.append((src_pos, src_pos + n, out_pos, out_pos + n))
            src_pos += n
            out_pos += n
        ...
        if field is not None:
            ...
            parts.append(value)
            if nested_entities:
                dynamic_entities.extend(nested_entities)
            out_n = len(value.encode('utf-16-le')) // 2
            src_pos += src_n
            out_pos += out_n

    rendered = ''.join(parts)
    out_entities = list(dynamic_entities)

    # Map only entities that are fully contained in one literal segment.
    # This is the critical part for Premium Emoji: the entity remains attached
    # to the exact emoji character after every placeholder has been expanded.
    for raw in entities or []:
        try:
            off = int(raw.get('offset', 0))
            length = int(raw.get('length', 0))
            end = off + length
            for src_a, src_b, out_a, out_b in literal_ranges:
                if src_a <= off and end <= src_b:
                    e = dict(raw)
                    e['offset'] = out_a + (off - src_a)
                    e['length'] = length
