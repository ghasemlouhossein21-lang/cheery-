...
def init_db():
    ...
    try:
        cur.execute("ALTER TABLE orders ADD COLUMN payment_method TEXT NOT NULL DEFAULT '-'")
    except Exception as exc:
        if "duplicate column" not in str(exc).lower():
            logging.getLogger(__name__).warning("migration orders.payment_method: %s", exc)
...
def create_order(
    ...
    referral_funded: bool = False,
    payment_method: str = "-",
) -> int:
    with transaction() as cur:
        cur.execute(
            """INSERT INTO orders
               (user_id, plan_key, plan_name, order_type, price, status, target_config_id, referral_funded, payment_method, created_at)
               VALUES (?, ?, ?, ?, ?, 'pending', ?, ?, ?, ?)""",
            (
                user_id, plan_key, plan_name, order_type, price, target_config_id,
                1 if referral_funded else 0, payment_method or "-", _now(),
            ),
        )
        return cur.lastrowid
