...
async def pay_with_wallet(callback: types.CallbackQuery, state: FSMContext):
    ...
    order_id = db.create_order(user["id"], plan_key, plan["name"], plan_type(plan_key), final_price, payment_method="کیف پول")
...
async def fulfill_free_test_directly(bot, message: types.Message, user: dict, plan: dict, plan_key: str) -> None:
    ...
    order_id = db.create_order(user["id"], plan_key, plan["name"], plan_type(plan_key), 0, payment_method="رایگان")
