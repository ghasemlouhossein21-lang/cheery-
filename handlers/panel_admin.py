...
async def _fulfill_custom_renew(bot, order: dict, order_id: int, volume, days) -> tuple[bool, str]:
    ...
    try:
        await send_rich(bot, ADMIN_ID, alerts.admin_delivery_summary(user, actual_name, alerts.get_config_package_name(cfg), order.get("price", 0), payment_method=((order or {}).get("payment_method") or "-")))
    except Exception:
...
async def _deliver_panel_link(bot, ctx: dict, link: str):
    ...
    try:
        ...
        admin_summary = alerts.admin_delivery_summary(user, name, admin_package_name, admin_amount, payment_method=((order_obj or {}).get("payment_method") or "-"))
