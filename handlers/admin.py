...
async def _log_fulfilled_order(
    ...
    # نام کاربری را از رکورد کاربر بخوان؛ تابع fetch_username در نسخه اصلی وجود ندارد
    # و فراخوانی آن باعث می‌شد کل لاگ قبل از رسیدن به کانال متوقف شود.
    username = user.get("username") or "-"

    payment_method = (order.get("payment_method") or "-") if order else "-"
...
async def approve_renewal(callback: types.CallbackQuery):
    ...
    try:
        await send_rich(callback.bot,ADMIN_ID,alerts.admin_delivery_summary(renew_user,service_username,package_name,int(receipt.get("amount") or 0), payment_method="کارت به کارت"))
    except Exception:
...
async def approve_crypto_payment(callback: types.CallbackQuery):
    ...
    if receipt.get("kind")=="crypto_renew":
        ...
        try:
            await send_rich(callback.bot,ADMIN_ID,alerts.admin_delivery_summary(renew_user,service_username,package_name,int(receipt.get("amount") or 0), payment_method="پرداخت ارزی"))
        except Exception:
