...
async def approve_purchase(callback: types.CallbackQuery):
    ...
    order_id = db.create_order(user["id"], plan_key, plan["name"], plan_type(plan_key), price, payment_method="کارت به کارت")
...
async def _log_fulfilled_order(
    ...
    # username را از همان رکورد کاربر می‌گیریم؛ تابع fetch_username در alerts
    # وجود ندارد و خطای آن قبلاً باعث می‌شد کل لاگ قبل از ارسال به کانال متوقف شود.
    username = user.get("username") or user.get("user_name") or None
    ...
    try:
        await send_rich(
            bot,
            ADMIN_ID,
            alerts.admin_delivery_summary(
                user,
                service_name,
                package_text,
                int(order.get("price") or 0) if order else 0,
                payment_method=payment_method,
            ),
        )
    except Exception:
        logger.exception("ارسال گزارش خرید/تحویل برای ادمین ناموفق بود")
...
async def request_fair_use(callback: types.CallbackQuery):
    ...
    if not cfg or cfg.get("deleted") or cfg.get("disabled"):
        await callback.answer("⚠️ این سرویس دیگر فعال نیست.", show_alert=True)
        return
    user = db.get_user_by_id(int(cfg.get("user_id") or 0))
    if not user or str(user.get("telegram_id")) != str(callback.from_user.id):
    ...
    try:
        ...
        fair_gb = 0
    if fair_gb <= 0:
        await callback.answer("⚠️ مصرف منصفانه در حال حاضر فعال نیست.", show_alert=True)
        return
    usage = {}
    try:
        sub_link = crypto.decrypt_config(cfg["config"])
        usage = await fetch_subscription_info(sub_link) or {}
    except Exception:
        logger.exception("خواندن مصرف برای درخواست Fair Use ناموفق بود")
    try:
        await alerts.send_fair_use_request_to_admin(callback.bot, user, cfg, fair_gb, usage)
        await send_rich(callback.bot, int(user["telegram_id"]), user_text("fair_use_selected"))
        await callback.answer("✅ درخواست شما برای ادمین ارسال شد.")
    except Exception:
        logger.exception("ارسال درخواست Fair Use به ادمین ناموفق بود")
        await callback.answer("❌ ارسال درخواست ناموفق بود. دوباره تلاش کنید.", show_alert=True)
...
async def approve_renewal(callback: types.CallbackQuery):
    ...
    try:
        ...
        await alerts.log_renewal_to_channel(callback.bot, renew_user, cfg or {}, panel_data, int(receipt.get("amount") or 0), volume, days, payment_method="کارت به کارت")
    except Exception:
...
async def approve_crypto_payment(callback: types.CallbackQuery):
    ...
    if receipt.get("kind")=="crypto_renew":
        ...
        try:
            renew_user=db.get_user(receipt["telegram_id"]) or {}
            await alerts.log_renewal_to_channel(callback.bot, renew_user, cfg or {}, panel_data, int(receipt.get("amount") or 0), volume, days, payment_method="پرداخت ارزی")
        except Exception:
    ...
    order_id=db.create_order(user["id"],plan_key,plan["name"],plan_type(plan_key),price,payment_method="پرداخت ارزی")
    db.resolve_pending_receipt_by_id(receipt_id)
...
async def admin_text_edit_save(message: types.Message, state: FSMContext):
    ...
    # در قالب‌های لاگ سفارش، متغیرها کاملاً اختیاری هستند؛ ادمین می‌تواند
    # هر متغیری را که نمی‌خواهد از متن حذف کند و لاگ همچنان ارسال شود.
    if not key.startswith("order_log_"):
